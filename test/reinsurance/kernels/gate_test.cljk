(ns reinsurance.kernels.gate-test
  ...
  3. governor boundary — the confidence floor boundary and the
     ...
     through the real `reinsurance.governor/check` façade."
...
(deftest bind-and-pay-auto-enabled-nowhere
  (testing "op 3 (:treaty/bind) and op 5 (:recovery/pay) are auto-enabled at
...
;; ---------------------------------------------------------------
;; Governor boundary through the real façade. op :treaty/intake
;; touches neither the store nor the spec/evidence/recovery checks, so
;; the verdict is decided purely by confidence/actuation — nil store
;; is safe.

(defn- verdict [proposal]
  (governor/check {:op :treaty/intake :subject "treaty-x"} {} proposal nil))
...
(deftest actuation-still-escalates-and-hard-still-wins
  ...
  (is (true? (:escalate? (verdict {:confidence 0.99 :stake :actuation/pay-recovery}))))
  (testing "a hard violation dominates actuation escalation"
    (let [v (governor/check {:op :jurisdiction/assess :subject "treaty-x"} {}
