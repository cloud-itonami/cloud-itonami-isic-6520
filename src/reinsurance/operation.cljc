(ns reinsurance.operation
  "OperationActor -- one reinsurance operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (Treaty-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the Reinsurance Governor (:govern) and the rollout phase gate
  ...
  One graph run = one reinsurance operation (intake -> advise -> govern
  ...
  decision to a human operator (the licensed reinsurance underwriter).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:treaty/bind`/`:recovery/pay` ALWAYS reach this node when
  the governor is clean -- see `reinsurance.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [reinsurance.treatyllm :as treatyllm]
...
(defn build
  ...
  `reinsurance.store/Store`).
  opts:
    :advisor      -- a `reinsurance.treatyllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (treatyllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Treaty-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (treatyllm/-advise advisor store request)]
            {:proposal p :audit [(treatyllm/trace request p)]})))

      ;; Reinsurance Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
